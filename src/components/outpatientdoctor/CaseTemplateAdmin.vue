...
<script>
import { listCaseTemplate, submitNewCaseTemplate, updateCaseTemplate } from '@/api/caseTemplate';
...
export default {
  ...
  methods: {
    ...
    addNewCaseTemplate() {
      this.dialogAddCaseTemplate = false;
      // 将新的病历模版放入当前病历中
      this.myCaseTemplates.push(JSON.parse(JSON.stringify(this.newCaseTemplate)));
    ...
    handleRemoveTemplates() {
      ...
      for (i = 0; i < this.selectedTemplates.length; i++) {
        this.myCaseTemplates.splice(
          this.myCaseTemplates.findIndex((template) => template.name === this.selectedTemplates[i].name),
          1
        );
      }
    },
