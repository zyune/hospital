...
export default {
  ...
  methods: {
    ...
    connection(roleId) {
      ...
      this.stompClient.connect(
        ...
        (frame) => {
          ...
          this.stompClient.subscribe('/topic/outpatient/queue/' + roleId, (response) => {
            ...
            if (data.code === 'update') {
              ...
              if (data.caseId === this.curPatient.caseId) {
                ...
                if (this.curPatient.caseId !== '') {
                  // 将当前被诊断的病人放回等待队列的开头
                  this.allPatients.unshift(Object.assign({}, this.curPatient));
                }
                // 如果该病人正在等待状态
                var index = this.allPatients.findIndex((patient) => patient.caseId === data.caseId);
                ...
                this.doTTS('请' + this.curPatient.name + '到' + this.curDoctor.name + '医生处就诊');
    ...
    queryDeparmentSearch(queryString, cb) {
      ...
      var results = queryString ? departments.filter(this.createDepartmentFilter(queryString)) : departments;
      cb(results);
    },
    ...
    queryDoctorSearch(queryString, cb) {
      ...
      var results = queryString ? doctors.filter(this.createDepartmentFilter(queryString)) : doctors;
      cb(results);
    },
